...
MapOptions parseOptions(int argc, char *argv[]) {
    ...
    if (!strlen(opts.ppmfn)) {
        ...
        //usage();
        //exit(-1);
        sprintf(opts.ppmfn, "images/%ld.ppm", opts.seed);
        sprintf(opts.pngfn, "images/%ld.png", opts.seed);
    }
...
int main(int argc, char *argv[]) {
    ...
    */
    char filename_noext[400];
    strncpy(filename_noext, opts.ppmfn, 400);
    int sl = strlen(filename_noext);
    // Remove ".ppm"
    sl -= 4;
    filename_noext[sl] = '\0';
    ...
    int i;
    for (i = 44; i > 0; i--) {
        printf("Generating map for layer %d\n", i);
        ...
        char cmd[800];
        snprintf(cmd, 800, "pnmtopng '%s' > '%s_%02d.png' && rm '%s'", opts.ppmfn, filename_noext, i - 1, opts.ppmfn);
        system(cmd);
